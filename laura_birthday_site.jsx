...
/* ---------------------------------------------------------
   LAURA — birthday website
   Palette: Lavender / Tan / Cream (from moodboard)
   Handwritten, slightly imperfect "scrapbook" typography.
   No phone mockup, no emoji, no "L" seal.
--------------------------------------------------------- */
...
const COUPONS = [
  ...
  },
  {
    ...
    subtitle: "unlimited uses",
    body: "Redeemable any day, any outfit, any haircut. I will always tell you that you look amazing — because you do.",
    code: "SISTERS-05",
  },
  {
    title: "Get-Out-Of-Trouble",
    subtitle: "one free pass",
    body: "Running late, forgot something, changed the plan last minute — this card means I won't say a thing about it.",
    code: "SISTERS-06",
  },
];
    ...
    try {
      ...
      if (!toneRef.current) {
        ...
        const synth = new Tone.PolySynth(Tone.Synth, {
          ...
          envelope: { attack: 1.2, decay: 0.6, sustain: 0.5, release: 3 },
          volume: -16,
        }).connect(reverb);
        ...
        const loop = new Tone.Loop((time) => {
          synth.triggerAttackRelease(chords[step % chords.length], "2n", time);
          step++;
        }, "2n");

        Tone.Transport.bpm.value = 60;
        toneRef.current = { Tone, synth, reverb, loop };
      }

      if (!musicOn) {
        toneRef.current.loop.start(0);
        toneRef.current.Tone.Transport.start();
        setMusicOn(true);
      } else {
        toneRef.current.loop.stop();
        toneRef.current.Tone.Transport.stop();
        setMusicOn(false);
      }
    } catch (e) {
  ...
  return (
    <div className="lb-root">
      ...
      <div className="lb-hero-wrap">
        ...
        <p className="lb-ribbon-sub">celebrating my favorite person since day one</p>
        ...
        <div className="lb-files-card">
          ...
          <div>
            ...
            <p className="lb-files-body">
              You know my silence just as well as my laugh. Thank you for being
              the one who makes every ordinary day feel like a core memory, and
              for being exactly, endlessly yourself.
            </p>
      ...
      <div className="lb-footer">
        ...
        <p className="lb-footer-sign">with love, your little sister</p>
      </div>
    </div>
  );
}
